...
for j = 1:length(time)
    ...
    ECI_stored(j,:)=ECI;
    Veci_stored(j,:)=Veci*1000;
    [ECEF,VCEF] = eci2ecef(ECI,GMST,Veci);
    ECEF_stored(j,:)=ECI;
    VCEF_stored(j,:)=VCEF*1000;
...
figure(5)
hold on
plot(time,ECI_stored(:,1))
plot(time,ECI_stored(:,2))
plot(time,ECI_stored(:,3))
plot(time,Veci_stored(:,1))
plot(time,Veci_stored(:,2))
plot(time,Veci_stored(:,3))
legend('X (km)','Y (km)','Z (km)','vx (km/s)','vy (km/s)','vz (km/s)')
xlabel('Time (s)')
ylabel('Distance')
hold off
print('ECIJ2000','-dpng')
...
figure(6)
hold on
plot(time,ECEF_stored(:,1))
plot(time,ECEF_stored(:,2))
plot(time,ECEF_stored(:,3))
plot(time,VCEF_stored(:,1))
plot(time,VCEF_stored(:,2))
plot(time,VCEF_stored(:,3))
legend('X (km)','Y (km)','Z (km)','vx (km/s)','vy (km/s)','vz (km/s)')
xlabel('Time (s)')
ylabel('Distance')
hold off
...
%     plotm(Lat(bp(j-1)+1:bp(j)),Long(bp(j-1)+1:bp(j)),s)
end
print('groundtrack','-dpng')
axis([-180,180,-90,90])

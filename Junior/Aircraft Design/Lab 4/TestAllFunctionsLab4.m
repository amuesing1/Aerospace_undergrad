...
load TestVariablesLab4
clear aircraft_parameters
recuv_tempest

%cost = AeroCostForTrim(trim_variables, trim_definition, aircraft_parameters);
%[force_moment_vec] = S_AeroForcesAndMoments([aircraft_state; control_input; wind_inertial; density], aircraft_parameters);
%[aero_forces, aero_moments] = AeroForcesAndMoments_BodyState_WindCoeffs(aircraft_state, control_input, wind_inertial, density, aircraft_parameters);
...
[aero_forces_check, aero_moments_check] = AeroForcesAndMoments_BodyState_WindCoeffs(aircraft_state, control_input, wind_inertial, density, aircraft_parameters);
...
[force_moment_vec_check] = S_AeroForcesAndMoments([aircraft_state; control_input; wind_inertial; density], aircraft_parameters);
...
%%%%%%%%%%%%%%%%%%%%%%%%%% AeroCostForTrim %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
disp('-')
disp('AeroCostForTrim')

cost_check = AeroCostForTrim(trim_variables, trim_definition, aircraft_parameters);


if (norm(cost - cost_check) > 1e-10)
    problems = problems+1;
    disp('*** has a problem in cost')
end
if problems == 0
    functions_good = functions_good+1;
    disp('works correctly')

function drawAircraft(uu,pts)
    ...
    if t<=0.1,
        ...
        figure(1), clf
        aircraft_handle1 = drawAircraftBody(pts.fuse, pn,pe,pd,phi,theta,psi,scale_plot, [], 'normal');
        aircraft_handle2 = drawAircraftBody(pts.wing, pn,pe,pd,phi,theta,psi,scale_plot, [], 'normal');
        aircraft_handle3 = drawAircraftBody(pts.tailwing, pn,pe,pd,phi,theta,psi,scale_plot, [], 'normal');
        aircraft_handle4 = drawAircraftBody(pts.tail, pn,pe,pd,phi,theta,psi,scale_plot, [], 'normal');
...
%=======================================================================
%
function handle = drawAircraftBody(NED,pn,pe,pd,phi,theta,psi, scale_plot, handle, mode)
    SCALE = scale_plot;
...
function handle = drawAirRelativeVelocity(position_inertial, air_rel_vel, euler_angles, scale_plot, handle, mode)
  SCALE = scale_plot/2;
  ...
  % plot spacecraft
  if isempty(handle),
    handle = plot3(XYZ(1,:),XYZ(2,:),XYZ(3,:),'ko-', 'LineWidth' ,2 , 'MarkerFaceColor', 'g', 'EraseMode', mode);
    hold on;
  else
    set(handle,'XData',XYZ(1,:),'YData',XYZ(2,:),'ZData',XYZ(3,:));
    drawnow

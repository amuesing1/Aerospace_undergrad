% Final code for Subgroup 2: Cooper Benson, Alan Sanchez

function truss3d_2(inputfile,outputfile)
...
%bvectotal is used to find input loads for the monte carlo simulation
bvectotal = zeros((size(joints,1)*3),1);

% compute forces in bars and reactions
n=10;
for i=1:n
[barforces,reacforces,bvec]=forceanalysis(joints,connectivity,reacjoints,reacvecs,loadjoints,loadvecs);
bvectotal=bvectotal+bvec;
max_force(i)=min(barforces);
end
...
%                              coefficient of varation = 0.083
%             joint position : 
%                              coefficient of varation = 0.005
...
% Plot the historgram of max bar forces
figure(1);
subplot(1,2,1);
hist(maxforces,30);
%axis([0,100,0,100])
...
% Plot histogram of maximum reactions
subplot(1,2,2);
hist(maxreact,30);
%axis([0,100,0,100]);
...
% Calculate failure rate due to error in bar length
...
function [barforces,reacforces,bvec]=forceanalysis(joints,connectivity,reacjoints,reacvecs,loadjoints,loadvecs)
...
for i=1:size(joints,1)
    ...
    joints(i,1)=joints(i,1)+0.005*randn(1);
    joints(i,2)=joints(i,2)+0.005*randn(1);
    joints(i,3)=joints(i,3)+0.005*randn(1);
end
